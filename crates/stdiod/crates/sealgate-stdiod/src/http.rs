...
//! token comes from ``config.toml`` (written by ``sealgate-stdiod login``).
...
pub fn from_config(cfg: &PersistedConfig) -> Result<BackendClient> {
    let token = cfg
        ...
        .ok_or_else(|| {
            anyhow!("no credential on disk. Run `sealgate-stdiod login --backend ...`.")
        })?;
    let backend = cfg.backend_url.clone().ok_or_else(|| {
        anyhow!("no backend URL on disk. Run `sealgate-stdiod login --backend ...`.")
    })?;
