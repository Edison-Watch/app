...
pub fn machine_installation_id() -> Option<String> {
...
mod machine_id_tests {
    ...
    #[test]
    fn machine_installation_id_is_stable_and_uuid_shaped() {
        let Some(a) = machine_installation_id() else {
            return; // no machine id on this host; nothing to assert
        };
        assert_eq!(Some(&a), machine_installation_id().as_ref());
    ...
    #[test]
    fn machine_installation_id_is_not_the_raw_machine_id() {
        let (Some(raw), Some(derived)) = (raw_machine_id(), machine_installation_id()) else {
            return;
        };
