...
//!
//! Discovery only: the servers in that file are read, but SealGate installs
...
//! flow with no config-file equivalent. SealGate used to bridge the gap by
...
impl Agent for ClaudeDesktop {
    ...
    /// Overridden because the default derives it from `sealgate_installs`, which
