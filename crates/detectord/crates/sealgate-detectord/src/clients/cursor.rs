...
/// skipping quarantined `sg-disabled-*` plugin dirs.
fn scan_plugin_cache(cache_dir: &Path) -> Vec<DiscoveredServer> {
    ...
    for market in subdirs(cache_dir) {
        for plugin in subdirs(&market) {
            ...
            if pname.starts_with("sg-disabled-") {
                continue;
            }
...
/// can't be fingerprinted/sent to SG — but they are **removable** by renaming
...
mod tests {
    ...
    /// the shadow-entry purge (which removes project-scoped sealgate entries)
    ...
    fn scans_plugin_cache_mcp_json() {
        ...
        let disabled = cache.join("acme-market").join("sg-disabled-old");
