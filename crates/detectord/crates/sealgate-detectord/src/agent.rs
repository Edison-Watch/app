...
pub trait Agent: Send + Sync {
    ...
    /// Whether SealGate can install the `sealgate` entry and inject hooks for
    ...
    /// config SealGate parses on every scan — see
    /// [`config_path`](Agent::config_path).
    ///
    /// Declared, not inferred from an empty
    /// [`sealgate_installs`](Agent::sealgate_installs): "no install target right
    ...
    /// tell the user it is there and outside SealGate's reach.
    ...
    /// Where/how to install the `sealgate` proxy entry for this agent, under
    ...
    /// different question from [`sealgate_installs`](Agent::sealgate_installs),
    /// which says where the `sealgate` entry may be *written*.
    ///
    /// The two coincide for most agents, hence the default. They come apart for
    /// a host SealGate can read but not install into, where deriving one from the
    /// other would deny a file the daemon parses on every scan.
    fn config_path(&self, home: &Path) -> Option<PathBuf> {
        self.sealgate_installs(home).first().map(|i| i.path.clone())
    }

    /// How to inject SealGate hooks for this agent under `home`, or `None`
    ...
    /// "SealGate Registration" folder-open task. Only VSCode uses this.
