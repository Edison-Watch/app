...
pub enum ServerConfig {
    ...
    /// fingerprinted or submitted to SealGate.
    ///
    ...
    /// though we can't move it to SG — so enforcement removes it. When `false`
...
/// How to install the `sealgate` proxy entry into an agent's config — the
/// inverse of quarantine (we *add* an entry). Produced by
/// [`Agent::sealgate_installs`](crate::Agent::sealgate_installs).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SealGateInstall {
...
/// The shape of an installed `sealgate` entry.
///
...
pub enum SealGateStyle {
    /// `{ "type": "http", "url": … }` in a JSON/JSONC file.
    Http,
    /// `[mcp_servers.sealgate]` in a TOML file (Codex).
    Toml,
}

/// How to inject SealGate hooks into an agent's config (phase-2 mirror of
/// [`SealGateInstall`]). The injected commands run scripts materialised into
/// `~/.sealgate/`; the scripts are self-contained (they only write files
/// there — no network).
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub enum HookStyle {
    ...
    CursorHooks,
    /// VSCode Copilot `~/.copilot/hooks/sealgate.json`: the whole file is
    /// SealGate-owned.
    CopilotFile,
...
pub enum HookScriptKind {
    /// `sealgate-hook.sh <client>` — writes a project-registration pending file.
    Registration,
    /// `sealgate-session-start.py` — persists the session id.
    SessionStart,
    /// `sealgate-session-hook.py` — tags MCP tool calls with the conversation id.
    SessionHook,
    /// `sealgate-session-end.py` — writes a session-end pending file.
    SessionEnd,
}
