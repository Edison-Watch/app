...
export default function AppsStep({
  ...
  const fullySupportedClients = clients.filter((c) => !CONNECTOR_BACKED_REASON.has(c.id));
  const partiallySupportedClients = clients.filter((c) => CONNECTOR_BACKED_REASON.has(c.id));
