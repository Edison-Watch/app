...
vi.mock("electron", () => ({
  app: {
    getPath: (name: string) => `/tmp/sealgate-test-${name}`,
    getVersion: () => "1.0.0-test",
  },
}));

// Mock @sealgate/shared/config (not resolvable in CI without workspace linking)
vi.mock("@sealgate/shared/config", () => ({
  getEnvByName: () => ({}),
}));
    ...
    mockExecFile.mockImplementation(
      (_cmd: string, _args: string[], _opts: unknown, cb: (err: Error & { stderr?: string }) => void) => {
        const err = Object.assign(new Error("Command failed: exit code 1"), {
          stderr: "No MCP server found with name: sealgate",
        });
        cb(err);
      },
    );
