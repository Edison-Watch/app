...
// re-run `sealgate-stdiod install`, whose bootout+bootstrap restarts the
...
// Written by `sealgate-stdiod install` - see stdiod/.../platform/macos.rs and
// LAUNCHD_LABEL in controller.ts.
const PLIST_RELATIVE_PATH = 'Library/LaunchAgents/com.sealgate.stdiod.plist'
...
/**
 * Re-run `sealgate-stdiod install` if the running launchd unit predates the
...
export async function maybeRefreshStdiodInstall(): Promise<void> {
  ...
  // packaged builds can hit update-induced skew. SG_STDIOD_REFRESH_TEST
  // mirrors SG_UPDATE_TEST for exercising this path unpackaged.
  if (!app.isPackaged && !process.env.SG_STDIOD_REFRESH_TEST) return
